...
setInterval(()=>{
  connection.query("SELECT 1", function(err){ if(err) console.error(err)});
}, 3* 1000);
...
const upLoadFileAsync = (path, filename, onfileHashed) => {
  fs.readFile(path + filename, (err, data) => {
    if (err) {
      ...
      var values = [filehash, filename];
      console.log(filename)
      var sql = connection.query("INSERT INTO file(hash, path) VALUES (?,?)", [...values], function(err, result){
        if(err){
	  console.error(err)
	  onfileHashed(null)
	  return;
...
router.post("/", uploadWithOriginalFilename.single("attachment"), (req, res, next) => {
  ...
  //fileName = fileName + uuidV1();
  console.log(fileName)
  upLoadFileAsync("uploadedFiles/", fileName, (hash) => {
    if (hash) {
